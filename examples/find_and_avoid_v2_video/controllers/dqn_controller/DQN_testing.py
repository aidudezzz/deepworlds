...
def run(experiment_name, env, deterministic, use_masking, testing_results_filename=None, tests_per_difficulty=100,
    ...
    load_path = experiment_dir + f"/{experiment_name}_diff_0_agent"
    ...
    try:
        model = DQN.load(load_path)  # NOQA
    except FileNotFoundError:
        load_path += ".zip"
        model = DQN.load(load_path)  # NOQA
    
    env.set_difficulty(difficulty_dict["diff_0"], "diff_0")
    print("################### SB3 EVALUATION STARTED ###################")
    print("Evaluating for 100 episodes in diff_0 (random map).")
